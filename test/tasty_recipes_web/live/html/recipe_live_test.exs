defmodule TastyRecipesWeb.Html.RecipeLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all recipes", %{conn: conn, recipe: recipe} do
      {:ok, _index_live, html} = live(conn, Routes.html_recipe_index_path(conn, :index))
    ...
    test "saves new recipe", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.html_recipe_index_path(conn, :index))

      assert index_live |> element("a", "New Recipe") |> render_click() =~
               "New Recipe"

      assert_patch(index_live, Routes.html_recipe_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.html_recipe_index_path(conn, :index))
    ...
    test "updates recipe in listing", %{conn: conn, recipe: recipe} do
      ...
      assert_patch(index_live, Routes.html_recipe_index_path(conn, :edit, recipe))
